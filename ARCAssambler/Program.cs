...
namespace AsamblorARC
{
    internal class Program
    {
        ...
        //Doesn't recognize labels
        //Implemented only: ld, st, jmpl, addcc, andcc / i'm not sure about the rest
        ...
        {
            ReadFile(@"C:\Users\bianc\source\repos\ARCAssambler\ARCAssambler\input.txt");
            FindVariables();
            line = org;
        ...
        private static void ConvertInstruction(string[] lineInstr, ref char[] result)
        {
            switch (lineInstr[0])
            {
                ...
                case "jmpl":
                    ...
                    result[19] = '0';
                    result[13] = '1';
                    SetInResult(lineInstr, ref result, 1, 18, 5); // rs1
                    SetInResult(lineInstr, ref result, 3, 12, 13);// rs2
                    SetInResult(lineInstr, ref result, 4, 29, 5); // destination
                    break;
                case "be":
                    result[31] = '0';
                    result[30] = '0';
        ...
        {
            SetInResult(instr, ref result, 1, 18, 5); // rs1
            SetInResult(instr, ref result, 2, 12, 13);// rs2
            SetInResult(instr, ref result, 3, 29, 5); // destination
        }
        ...
        {
            //position increases beacause we read instruction ld/st
            //set rs1 - 0
            for (int i = 0; i < 6; i++)
            {
                result[18 - i] = '0';
            }
            // read first directory from which we load/store
            if (instr[0] == "ld")
            {
                SetInResult(instr, ref result, 1, 12, 13); //rs2
                SetInResult(instr, ref result, 2, 29, 5); // directory
            }
            else
            {
                SetInResult(instr, ref result, 2, 12, 13); //rs2
                SetInResult(instr, ref result, 1, 29, 5); // directory
            }
        }
        private static void SetInResult(string[] instr, ref char[] result, int pozinput, int pozResult, int lenght)
        {
            if (!(int.TryParse(instr[pozinput], out _))) // verificam daca e un numar
            {
                if (instr[pozinput][1] == 'r') // is the destination a repositry? %r_ 
                {
                    if (result[13] != '1') result[13] = '0';
                    if (instr[pozinput].Length == 4) // if the repo is only one number, not ex 10
                    {
                        string nr = "";
                        nr += instr[pozinput][2];
                        nr += instr[pozinput][3];
